...
with io.open(filename, newline='') as csv_file:
	...
	# Split into multiple CSVs based off of the values chosen
	# Once a value has been selected, allow the user to exit the loop or continue selecting values.
	for value in unique_value:
		# Create a new file named after the choices[value] array member
		with open('./{}_split.csv'.format(choices[value]), 'w') as newfile:
